...
class MetaApiInterface(ExchangeInterface):
    ...
    def get_product_history(self, symbol: str, epoch_start: float, epoch_stop: float, resolution: int):
        ...
        try:
            try:
                mtaccount = asyncio.run(self.calls.metatrader_account_api.get_account('5b5582fc-87b0-4f43-a437-79bd1b4cd706'))
                bars = mtaccount.get_historical_candles(symbol = symbol, start_time = epoch_stop_str)
                print(bars)
            except TypeError:
